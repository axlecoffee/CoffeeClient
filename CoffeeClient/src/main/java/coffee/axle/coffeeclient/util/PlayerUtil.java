...
public class PlayerUtil {
    ...
    public static boolean isSneaking() {
        return mc.currentScreen == null && mc.gameSettings.keyBindSneak.isKeyDown();
    }

    public static boolean isMovingLeft() {
        return mc.currentScreen == null && mc.gameSettings.keyBindLeft.isKeyDown();
    }

    public static boolean isMovingRight() {
        return mc.currentScreen == null && mc.gameSettings.keyBindRight.isKeyDown();
    }

    public static boolean isAttacking() {
        return mc.currentScreen == null && mc.gameSettings.keyBindAttack.isKeyDown();
    }

    public static boolean isUsingItem() {
        return mc.currentScreen == null && mc.gameSettings.keyBindUseItem.isKeyDown();
    }
    ...
    public static boolean canFly(int checkHeight) {
        if (!mc.thePlayer.capabilities.allowFlying && !mc.thePlayer.capabilities.disableDamage) {
            ...
            for (int offset = 0; offset <= checkHeight; ++offset) {
                ...
                Block block = mc.theWorld.getBlockState(new BlockPos(mc.thePlayer.posX, currentY, mc.thePlayer.posZ))
                        .getBlock();
    ...
    public static boolean canReach(BlockPos blockPos, double reach) {
        return isBlockWithinReach(blockPos, mc.thePlayer.posX, mc.thePlayer.posY + (double) mc.thePlayer.getEyeHeight(),
                mc.thePlayer.posZ, reach);
    }
    ...
    public static void attackEntity(Entity target) {
        if (ForgeHooks.onPlayerAttackTarget(mc.thePlayer, target)) {
            if (target.canAttackWithItem() && !target.hitByEntity(mc.thePlayer)) {
                float baseDamage = (float) mc.thePlayer.getEntityAttribute(SharedMonsterAttributes.attackDamage)
                        .getAttributeValue();
                float enchantmentBonus = EnchantmentHelper.getModifierForCreature(
                        mc.thePlayer.getHeldItem(),
                        target instanceof EntityLivingBase ? ((EntityLivingBase) target).getCreatureAttribute()
