...
import coffee.axle.coffeeclient.util.ColorUtil;
...
public class HUD extends Feature {
    ...
    private List<Feature> activeFeatures = new ArrayList<>();
    ...
    private String getFeatureName(Feature feature) {
        ...
        if (lowerCase.getValue()) {
            featureName = featureName.toLowerCase(Locale.ROOT);
        }
        return featureName;
    }

    private String[] getFeatureSuffix(Feature feature) {
        return new String[0];
    }

    private int getFeatureWidth(Feature feature) {
        return calculateStringWidth(getFeatureName(feature), getFeatureSuffix(feature));
    }
    ...
    public void updateActiveFeatures() {
        if (isEnabled() && CoffeeClient.featureManager != null) {
            activeFeatures = CoffeeClient.featureManager.features.values().stream()
                    .filter(feature -> feature.isEnabled() && !feature.isHidden())
                    .sorted(Comparator.comparingInt(this::getFeatureWidth).reversed())
    ...
    public void onRenderOverlay(RenderGameOverlayEvent.Post event) {
        ...
        for (Feature feature : activeFeatures) {
            ...
            String[] featureSuffix = getFeatureSuffix(feature);
            float totalWidth = (float) (calculateStringWidth(featureName, featureSuffix) - (shadow.getValue() ? 0 : 1));
