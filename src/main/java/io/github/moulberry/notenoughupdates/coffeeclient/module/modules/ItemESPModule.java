...
public class ItemESPModule extends Module {
    ...
    public final IntProperty opacity = new IntProperty("opacity", 25, 0, 100);
    ...
    private boolean shouldHighlightItem(int itemId) {
        return this.emeralds.getValue() && this.isEmeraldItem(itemId) ||
                ...
                this.gold.getValue() && this.isGoldItem(itemId) ||
    ...
    public void onRenderWorld(RenderWorldLastEvent event) {
            ...
            if (entity.ticksExisted >= 3 &&
                ...
                if (stack.stackSize > 0) {
                    ...
                    if (this.shouldHighlightItem(itemId)) {
                        double x = RenderUtil.lerpDouble(entityItem.posX, entityItem.lastTickPosX,
                                event.partialTicks);
                        double y = RenderUtil.lerpDouble(entityItem.posY, entityItem.lastTickPosY,
                                event.partialTicks);
                        double z = RenderUtil.lerpDouble(entityItem.posZ, entityItem.lastTickPosZ,
                                event.partialTicks);
        ...
        for (Entry<ItemData, Integer> itemEntry : itemMap.entrySet().stream()
            ...
            double x = itemEntry.getKey().x - mc.getRenderManager().viewerPosX;
            double y = itemEntry.getKey().y - mc.getRenderManager().viewerPosY;
            double z = itemEntry.getKey().z - mc.getRenderManager().viewerPosZ;
            double distance = mc.getRenderViewEntity().getDistance(itemEntry.getKey().x, itemEntry.getKey().y,
                    itemEntry.getKey().z);
